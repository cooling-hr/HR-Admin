...
## نتيجة التنفيذ (2026-09-23)

المراحل السبع اكتملت. الانحرافات عن نصّ الخطة، ولماذا:

- **المهمة 5 كانت أكبر بكثير مما وُصفت.** الخطة قالت «تعليقات ومسافات ونصوص»؛ الواقع 93 موضعاً
  منها نحو 30 فرقاً سلوكياً (دور manager، قفل الأقسام، سجل التدقيق، حزمة المزامنة، صلاحيات اللقطات،
  معرّفات Firebase). عولجت هكذا:
  - فرع سحابي **ميت في الأوفلاين** (لا دور manager أوفلاين إطلاقاً؛ مركز الاستعادة للمدير وحده؛
    `lockedSections` كائن فارغ ثابت؛ `logAuditEvent` لا شيء؛ `pushDataToCloud` يعود فوراً لأن الرابط
    فارغ) ⇒ تبنّت الأوفلاين نصّ السحابية حرفياً من الـdiff نفسه.
  - سلوك يختلف فعلاً ⇒ اسم واحد في طبقة كل نسخة: `authorizeEmployeeDelete`، `authorizeDirectWipe`،
    `authorizeWipeApproval`، `sessionKeyFor`، `isSelfUser` (الأخير يعيد `false` أوفلاين عمداً: تبنّي
    المقارنة السحابية مباشرة كان سيُخفي زر فك القفل عن صفّ المدير نفسه).
  - نصوص تذكر Firebase، وقائمة الأدوار، ولوحة رمز مدير الجهاز، وزر كشف كلمات المرور ⇒ AuthViews
    (`editionTexts`، `RoleOptions`، `DeviceAdminPinPanel`، `RevealPinsToggle`).
  - التفكيك ووسائط الـhook ⇒ اتحاد القائمتين؛ ما لا توفّره نسخة يصل undefined ولا يقرؤه إلا مكوّن
    AuthViews للنسخة الأخرى. طبقة السحابية تعيد الرابط بالمفتاح `fb_DB_URL` (اسم الخاصية ينجو من
    التصغير، والأوفلاين يجب ألا تحمل كلمة firebase).
- **المهمة 6: مصنع لا prop.** `createStaffSystem({ useDataLayer, AuthViews })` يُستدعى مرة عند
  التحميل (اقتراح Antigravity) — هوية الـhook ثابتة فلا يتبدّل ترتيب الـhooks. لا prop اسمه `edition`:
  لم يبقَ في الجسم أي شرط على النسخة.
- **زلّة أُمسكت قبل الالتزام:** الخطة أبقت `ReactDOM` في المدخل وحده، لكن الجسم يستعمل
  `ReactDOM.createPortal` لبطاقة الموظف. البناء نجح رغم ذلك؛ فحص الأسماء غير المحلولة كشفه.
- **البصمة:** السحابية ثابتة (1658). الأوفلاين لم تفقد نصاً وزادت 8 كلها داخل فروع لا تُرسَم فيها.
- **ملاحظتا Codex مؤجَّلتان (سابقتان للدمج، لا تراجع منه):** مؤثّر Enter في شاشة الترحيب لا يضع
  `proceedFromWelcome` في اعتمادياته؛ وحارس بذرة الأولويات يعتمد على `ref.current` في الاعتماديات
  (آمن أوفلاين، ونظرياً قد يتأخر سحابياً إن اكتمل التحميل دون تغيّر `staff`).

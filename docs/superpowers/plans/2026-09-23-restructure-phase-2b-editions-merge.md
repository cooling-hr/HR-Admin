...
- **لا اختبارات آلية** (قرار المستخدم 2026-09-16). التحقق: فحص الأنواع + بناء
  النسختين + `check-shell.mjs` + بصمة النصوص + مراجعة Codex + تجربة المستخدم.
...
| `HeaderExtras` | إضافات الشريط العلوي (شارة المزامنة، قائمة المستخدم) |
| `Overlays` | الطبقات الكاملة: شاشة الرمز الرباعي، نافذة المزامنة، تولّي الجلسة |

نسخةٌ لا تحتاج مكوّناً تُمرّر `() => null` — لا شرط في الجسم المشترك.

...
// الأوفلاين لا تتصل بأي شيء. طبقتها منسوخة أصلاً من محرّك السحابة بروابط مُفرغة، فبقيت فيها
// نحو عشرين محاولة اتصال — أربع منها حلقات دورية كل 3–5 ثوانٍ — تفشل كلها وتملأ وحدة التحكم
// بالأخطاء، ومنها مسار خادم قديم (/api/sync) كان سيتصل فعلاً لو استُضيف الملف على خادم.
// هذا البديل يحجب الشبكة كلها من نقطة واحدة داخل هذه الوحدة وحدها: يرفض كما كان الاتصال الحقيقي
// يرفض هنا أصلاً، فتسلك الشيفرة مساراتها نفسها بلا طلب فعلي. معالج الرفض المُلحَق يمنع تقرير
// «رفض غير معالَج» للمواضع التي لا تنتظر النتيجة (الخروج، إغلاق التبويب)، ولا يمنع من ينتظرها
// من تلقّي الرفض في catch الخاص به.
const fetch = () => {
    const p = Promise.reject(new Error('offline edition: no network'));
    p.catch(() => {});
    return p;
};

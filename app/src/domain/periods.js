...
export const PERIOD_TYPES = ['إجازة اعتيادية', 'في دورة', 'إيفاد', 'إجازة بدون راتب', 'إجازة طويلة', 'إجازة أمومة', 'غياب', 'سحب يد'];
...
export const OPEN_ENDED_PERIOD_TYPES = ['غياب', 'سحب يد'];
...
export const QUICK_STATUS_OPTIONS = ['نشط', 'في دورة', 'إجازة طويلة', 'إجازة أمومة', 'غياب', 'سحب يد'];

export const periodsOf = (emp) => (emp && Array.isArray(emp.statusPeriods)) ? emp.statusPeriods : [];
...
export const getActivePeriod = (emp, dateStr) => {
...
export const periodEndOf = (p) => (p && p.to) ? p.to : '9999-12-31';

// تداخل تواريخ صارم: النهاية الفارغة مفتوحة، والفترتان المتتاليتان لا تتداخلان
export const periodsOverlap = (a, b) => a.from <= periodEndOf(b) && b.from <= periodEndOf(a);

// هوية الفترة: معرّفها، أو نوعها وتواريخها إن كانت قديمة بلا معرّف — فلا تتطابق فترتان لمجرد غياب معرّفيهما
export const periodIdentityOf = (p) => p.id || ('~' + [p.type, p.from, p.to || ''].join('|'));

export const periodMergeKeyOf = (jobNumber, p) => `${jobNumber}::${periodIdentityOf(p)}`;

export const samePeriodDates = (a, b) => a.type === b.type && a.from === b.from && (a.to || '') === (b.to || '');

export const samePeriodExtras = (a, b) => (a.note || '') === (b.note || '') && !!a.confirmedReturn === !!b.confirmedReturn;

export const periodPhaseOf = (p, today) => (p && p.to && p.to < today) ? 'منتهية' : ((p && p.from > today) ? 'قادمة' : 'سارية');

// النهاية تُحتسب من المدة (يوم البداية محسوب ضمنها) أو تُؤخذ من تقويم النهاية مباشرة
export const quickPeriodEnd = (draft) => {
...
export const isLongOrMaternityLeave = (statusStr) => {

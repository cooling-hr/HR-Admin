...
def update_file(file_path):
    ...
    # Replace Profile Modal with Smart Dynamic Layout & Apply Customization Button
    old_profile_modal = """            {/* نافذة معاينة وبطاقة ملف المنتسب الفاخرة قبل الطباعة مع خيارات التخصيص والإخفاء والإظهار */}
    ...
    new_profile_modal = """            {/* نافذة معاينة وبطاقة ملف المنتسب الفاخرة قبل الطباعة مع خيارات التخصيص والإخفاء والإظهار التلقائي التنسيق */}
            {selectedEmployeeCard && (
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-3 sm:p-4 print:p-0 print:bg-white print:static">
                    <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 max-w-2xl w-full max-h-[92vh] flex flex-col overflow-hidden animate-fadeInUp print:shadow-none print:border-none print:max-w-none print:w-full print:max-h-none">
                        {/* Header */}
    ...
    if old_profile_modal in code:
        ...
        print(f"✓ Applied Smart Dynamic Field Layout & Customization Apply Button to {file_path}")
